...
//					cout << "zbin " << zbin << " epbins[" << i << "][" << j << "] pTtot: " << epbins[i][j].pTtot << endl;
	...
	//Now merge clusters, if necessary
        	for(int m = 0; m < nclust -1; ++m){
                        ...
                        if(L2cluster[n].eta == L2cluster[m].eta && (fabs(L2cluster[n].phi - L2cluster[m].phi) < 1.5*phistep)){
	...
	//	for(int db=0;db<nclust;++db)cout<<all_zbins[zbin].clusters[db].phi<<"\t"<<all_zbins[zbin].clusters[db].pTtot<<"\t"<<all_zbins[zbin].clusters[db].numtracks<<endl;	
